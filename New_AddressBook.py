...
class AddressBook:
    ...
    def file_reader(self):
        try:
            file = open('/Users/kyeonghayun/Documents/Tutorials/참조/python/addressBook.csv', 'rt',encoding='cp949')  
            print(file)
            # addrBook.csv 파일이 없으면 예외 발생
    ...
    def update(self):
        ...
        updated = False
        for i, person in enumerate(self.address_list):
            print(' enumerate(self.address_list) ' , enumerate(self.address_list))
            if name == self.address_list[i].name:
                print('검색된 전화번호가 "{}"입니다.'.format(self.address_list[i].phone))
